...
export function activate(context: vscode.ExtensionContext) {
	try {
		outputChannel = vscode.window.createOutputChannel("億までミエール");
		outputChannel.appendLine('拡張機能 "oku-made-mieru" が有効になりました。');
		outputChannel.show();
		...
		vscode.window.onDidChangeTextEditorSelection((event) => {
			...
			if (!selection.isEmpty) {
				...
				try {
					const japaneseNumber = converter.convert(text);
					statusBarItem.text = `日本語表記: ${japaneseNumber}`;
					outputChannel.appendLine(`日本語表記: ${japaneseNumber}`);
				} catch (error) {
					statusBarItem.text = '日本語表記: 変換できません';
					outputChannel.appendLine('日本語表記: 変換できません');
				}
			} else {
				statusBarItem.text = '日本語表記: テキストを選択してください';
				outputChannel.appendLine('日本語表記: テキストを選択してください');
			}
